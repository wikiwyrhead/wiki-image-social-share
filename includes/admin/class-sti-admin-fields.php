        ...
        private function generate_field_types($value)
        {
            ...
            switch ($value['type']) {
                ...
                case 'display_rules':
                    ...
                    // Ensure we have a valid default rule before creating the class
                    $default_rule_data = null;
                    if (isset($rules['common']) && is_array($rules['common']) && !empty($rules['common'])) {
                        $default_rule_data = $rules['common'][0];
                    } else {
                        // Fallback default rule if common rules are not available
                        $default_rule_data = array(
                            "name" => __("Selector", "wiki-image-social-share"),
                            "id"   => "selector",
                            "type" => "text",
                            'placeholder' => __("Image css selector", "wiki-image-social-share") . ' ( default = img )',
                            "operators" => "equals",
                        );
                    }

                    // Only create the class if we have valid rule data
                    if (class_exists('STI_Admin_Display_Rules') && $default_rule_data) {
                        $default_rule = new STI_Admin_Display_Rules($default_rule_data);
                    } else {
                        // Log error for debugging
                        error_log('WISS: STI_Admin_Display_Rules class not found or invalid rule data');
                        break; // Skip this field type if class is not available
                    }
                ...
                case 'heading': ?>
                    ...
                    <tr valign="top" class="heading<?php echo $container; ?>">
                        <th scope="row">
                            <<?php echo $tag; ?>><?php echo esc_html($value['name']); ?></<?php echo $tag; ?>>
                        </th>
                        <td>
