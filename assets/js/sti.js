...
(function ($) {
  ...
  StiHooks.apply_filters = function (tag, value, options) {
    ...
    if (
      typeof StiHooks.filters[tag] !== "undefined" &&
      StiHooks.filters[tag].length > 0
    ) {
  ...
  $.fn.sti = function (options) {
    ...
    var appendButtonsTo = StiHooks.apply_filters(
      "sti_append_buttons_to",
      "body"
    );
    ...
    var methods = {
      ...
      scrollToImage: function (el) {
        ...
        var hash =
          typeof urlParam["scroll"] !== "undefined" ? urlParam["scroll"] : "";
      ...
      shareButtons: function () {
        ...
        var buttons = methods.isMobile()
          ? opts.buttons.mobile
          : opts.buttons.desktop;

        if (buttons) {
          for (var i = 0; i < buttons.length; i++) {
            var network = buttons[i];
            buttonsList +=
              '<div class="sti-btn sti-' +
              network +
              '-btn" data-network="' +
              network +
              '" rel="nofollow">';
      ...
      getSvgIcon: function (network) {
        ...
        if (
          opts.custom_data &&
          opts.custom_data.buttons &&
          opts.custom_data.buttons[network]
        ) {
      ...
      showShare: function (e, box, relative) {
        e = StiHooks.apply_filters("sti_share_container", e, {
          box: box,
          opts: opts,
          relative: relative,
        });
      ...
      detectRightContainer: function (el) {
        ...
        if (methods.isMobile() && el.nodeName === "IMG") {
          if (
            e[0].naturalWidth < opts.minWidth ||
            e[0].naturalHeight < opts.minHeight
          )
            return false;
        } else {
          if (e.width() < opts.minWidth || e.height() < opts.minHeight)
            return false;
        }
      ...
      setBoxLayout: function (box, relative) {
        ...
        if (offset && parentOffset) {
          ...
          if (parentPosition === "relative") {
            top =
              offset.top - parentOffset.top + parseInt(e.css("padding-top"));
            left =
              offset.left - parentOffset.left + parseInt(e.css("padding-left"));
          } else {
          ...
          if (
            opts.zIndex &&
            opts.zIndex !== "9999999999999999" &&
            !methods.isMobile() &&
            opts.position === "image_hover"
          ) {
            styles.zIndex = opts.zIndex;
          }

          styles = StiHooks.apply_filters("sti_sharing_box_layout", styles, {
            box: box,
            el: e,
            opts: opts,
          });
      ...
      replaceChars: function (string) {
        ...
        if (str && str !== "") {
          ...
          specialCharsRegex = StiHooks.apply_filters(
            "sti_chars_remove_regex",
            specialCharsRegex,
            { str: str }
          );
      ...
      shareData: function (el, network) {
        var data = {},
          e = currentImage
            ? $(currentImage)
            : currentImageElements && $(el).closest(".sti-top").data("el")
            ? currentImageElements[$(el).closest(".sti-top").data("el")]
            : $(el).closest(".sti-container")
            ? $(el).closest(".sti-container")
            : false,
          caption = false,
          captionText = false;
        ...
        data.media = e.data("media")
          ? e.data("media")
          : e[0].src
          ? e[0].src
          : methods.getBgImageURL(e);

        if (typeof data.media === "undefined") {
          var insideE = methods.findMediaElement(e);

          if (insideE) {
            data.media = insideE.data("media")
              ? insideE.data("media")
              : insideE[0].src
              ? insideE[0].src
              : methods.getBgImageURL(insideE);
            e = insideE;
          }
        }
        ...
        data.media = StiHooks.apply_filters(
          "sti_media",
          data.media,
          e,
          network
        ...
        data.summary = e.data("summary")
          ...
          ? opts.summary
          : "";
        // Enhanced message formatting for different networks
        if (data.network === "twitter") {
          // Clean Twitter format: Title only (image will be shown via Twitter Card)
          data.message = data.title;
        } else if (data.network === "whatsapp") {
          // WhatsApp format: Title + description + link (no direct image URL)
          data.message = data.title + (data.summary ? "\n" + data.summary : "");
        } else if (data.network === "linkedin") {
          // LinkedIn format: Professional title only (image via Open Graph)
          data.message = data.title;
        } else {
          // Default format for other networks
          data.message = data.title + " " + data.media;
        }
        data.local = location.href
          .replace(/\?img.*$/, "")
          .replace(/\&img.*$/, "")
          .replace(/#.*$/, "")
          .replace(/[\?|&]scroll.*$/, "");
      ...
      getUrlHash: function (data) {
        if (
          $(data.e).closest(".et_pb_gallery_item").length > 0 ||
          $(data.e).closest(".mfp-container").length > 0
        ) {
      ...
      findMediaElement: function (e) {
        ...
        if (
          e.find(".coin-slider").length &&
          e.find(".coin-slider .cs-title + a").length &&
          e.find(".coin-slider .cs-title + a").css("background-image")
        ) {
        ...
        switch (network) {
          ...
          case "messenger":
            if (!methods.isMobile()) {
              ...
              url +=
                "&redirect_uri=" +
                encodeURIComponent(data.local + data.schar + "close=1");
            } else {
          ...
          case "twitter":
            ...
            // Enhanced Twitter format: Clean text with proper URL
            var twitterText = data.title;
            if (data.summary && data.summary !== data.title) {
              twitterText += " - " + data.summary;
            }
            url += "text=" + encodeURIComponent(twitterText);
            url += "&url=" + encodeURIComponent(data.page);
            if (opts.twitterVia) {
            ...
            if (data.hashtags) {
              url += "&hashtags=" + encodeURIComponent(data.hashtags);
            }
            break;

          case "whatsapp":
            url += "https://api.whatsapp.com/send?";
            // Enhanced WhatsApp format with proper message structure
            var whatsappMessage = "";
            if (data.title) {
              whatsappMessage += data.title;
            }
            if (data.summary && data.summary !== data.title) {
              whatsappMessage += "\n\n" + data.summary;
            }
          ...
          case "download":
            ...
            methods.downloadImage(data.media, data.title);
            return; // Don't open popup for download
            break;
        }
      ...
      openPopup: function (url, data) {
        var win = window.open(
          url,
          "Share This Image",
          data.w_size + ",status=0,toolbar=0,menubar=0,scrollbars=1"
        );
        var timer = setInterval(function () {
          if (win.closed) {
            clearInterval(timer);
            methods.createAndDispatchEvent(document, "stiSharingWindowClosed", {
              url: url,
              data: data,
            });
          }
      ...
      downloadImage: function (imageUrl, imageName) {
        // Enhanced download functionality with server-side proxy support
        var fileName = imageName || "image";
        ...
        methods.createAndDispatchEvent(document, "stiImageDownload", {
          imageUrl: imageUrl,
          imageName: fileName,
        });
      },

      tryDownloadMethods: function (imageUrl, fileName, methods_array) {
        var currentMethod = methods_array.shift();
      ...
      analytics: function (category, label) {
        methods.createAndDispatchEvent(document, "stiAnalytics", {
          button: category,
          image: label,
        });
      ...
      relayoutButtons: function () {
        if (
          opts.position === "image" ||
          (opts.position === "image_hover" && methods.isMobile())
        ) {
      ...
      setShortLinks: function (data) {
        ...
        if (
          ...
          allowedNetworks.indexOf(data.network) != -1
        ) {
      ...
      storeImageMetadata: function (
        ...
        $.ajax({
          ...
          data: {
            ...
            page_url: pageUrl,
            network: network,
            nonce: opts.store_meta_nonce || "",
          },
          success: function (response) {
            if (response.success) {
              console.log("Image metadata stored for hash:", imageHash);
            } else {
              console.warn("Failed to store image metadata:", response.data);
            }
          },
          error: function (xhr, status, error) {
            console.warn("Error storing image metadata:", error);
          },
        });
      },
    };
    ...
    if (!methods.isMobile()) {
      if (opts.position !== "image_hover") {
          ...
          if (el && !el.next().hasClass("sti")) {
            ...
            el.after(
              '<div data-el="' +
                ...
                '" id="sti-box-' +
                i +
                '" class="sti sti-top style-flat-small sti-inside" style="display: none;"><div class="sti-share-box">' +
        ...
        if (!$("#" + stiBoxSingleSelector).length) {
          $(appendButtonsTo).append(
            '<div id="' +
              stiBoxSingleSelector +
              '" class="sti sti-top sti-hover style-flat-small" style="display: none;"><div class="sti-share-box">' +
              methods.shareButtons() +
              "</div></div>"
          );
        }
      ...
      if (!$("#" + stiBoxSingleSelector).length) {
        $(appendButtonsTo).append(
          '<div id="' +
            stiBoxSingleSelector +
            '" class="sti sti-top sti-mobile style-flat-small" style="display: none;"><div class="sti-share-box">' +
            methods.shareButtons() +
            "</div></div>"
        );
      }
        ...
        if (el && !el.next().hasClass("sti-mobile-btn")) {
          ...
          el.after(
            '<div data-el="' +
              ...
              '" id="sti-mobile-btn-' +
              i +
              '" class="sti-top sti-mobile-btn" style="display: none;">' +
              methods.getSvgIcon("mobile") +
              "</div>"
          );
    ...
    $('.sti-btn, a[href^="#sti-"]').on("click touchend", function (e) {
      ...
      methods.createAndDispatchEvent(this, "stiButtonClick", {
        button: network,
        data: data,
      });
    });
    ...
    $(window).scroll(function () {
      if (
        opts.position === "image_hover" &&
        !methods.isMobile() &&
        $("#" + stiBoxSingleSelector).is(":visible")
      ) {
  ...
  $(document).on(
    "shown.simplelightbox doLightboxViewImage modula_lightbox2_lightbox_open theiaPostSlider.changeSlide, envirabox-change, elementor/popup/show",
    function () {
      setTimeout(function () {
        $(selector).sti();
      }, 500);
  ...
  $(window).on("load", function () {
    ...
    var element = false;
    var lightboxesSelector =
      ".nivo-lightbox-image:visible, .slb_content:visible, .mfp-img:visible, #envirabox-img:visible";
    var watchToElements =
      ".wp-block-jetpack-slideshow, .jp-carousel-wrap, .wp-block-envira-envira-gallery, .slb_viewer, .sl-wrapper.simple-lightbox, .elementor-lightbox .dialog-lightbox-message";
      ...
      $imageLinks.on("click", function () {
        ...
        setTimeout(function () {
          ...
          if ($lightboxes.length > 0) {
            $(selector).sti();
            if (
              typeof StiHooks === "object" &&
              typeof StiHooks.add_filter === "function"
            ) {
              element = $(currentImg);
